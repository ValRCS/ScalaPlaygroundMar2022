...
object Day15CaseClassPatternMatching extends App {
  ...
  //since all three SMS, VoiceRecording and Email extend Notification
  // we can pass these
  // created objects to functions that take Notification as a parameter

  //I could not pass SMS to function that requires VoiceRecording
  ...
  def showImportantNotification(notification: Notification, importantPeopleInfo: Seq[String]): String = {
    notification match {
      ...
      case SMS(number, _) if importantPeopleInfo.contains(number) =>
        s"You got an SMS from special someone! That special someone is at number $number"
      case other =>
